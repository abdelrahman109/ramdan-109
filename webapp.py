...
from app.services import (
    ...
    validate_for_checkin,
    checkin,
    get_booking_by_qr_token,
)
from app.notifications import (
    send_ticket_message,
    send_thank_you_message,
    send_rejected_message,
    send_broadcast,
)
...
def admin_cancel_booking(booking_id):
    ...
    cancel_booking(booking_id, admin_name="admin")
    flash("تم إلغاء الحجز بنجاح")
    return redirect(request.referrer or url_for("admin_bookings"))
    ...
    if request.method == "POST":
        ...
        if message:
            with connect() as conn:
                if target == "attendees":
                    rows = conn.execute(
                        "SELECT telegram_chat_id FROM bookings "
                        "WHERE is_attending=1 AND status IN ('paid','used') "
                        "AND telegram_chat_id IS NOT NULL"
                    ).fetchall()
                elif target == "contributors":
                    rows = conn.execute(
                        "SELECT telegram_chat_id FROM bookings "
                        "WHERE is_attending=0 AND status='paid' "
                        "AND telegram_chat_id IS NOT NULL"
                    ).fetchall()
                else:
                    rows = conn.execute(
                ...
                conn.execute(
                    ...
                    "(message_title, message_body, target_group, sent_count, sent_by, sent_at) "
                    "VALUES (?, ?, ?, ?, ?, datetime('now'))",
